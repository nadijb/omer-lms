...
const ADMIN_NAV = [
  ...
  {
    href: "/lms/admin/physical-training",
...
export default function LMSLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user?.role === "trainer" && pathname === "/lms")
      router.push("/lms/trainer");
    ...
    <div className="flex h-screen bg-cortex-bg overflow-hidden">
      ...
      <aside className="w-60 flex-shrink-0 bg-cortex-surface border-r border-cortex-border flex flex-col">
        ...
        <div className="px-5 py-4 border-b border-cortex-border flex items-center gap-3">
          ...
          <div>
            <div className="text-cortex-text font-semibold text-sm leading-tight">
              Cortex LMS
            </div>
            <div className="text-cortex-muted text-[10px] uppercase tracking-wider">
              {user.role}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-cortex-border space-y-1">
          ...
          <div className="px-3 py-1 text-[11px] text-cortex-muted truncate">
            {user.email}
          </div>
          <button
            onClick={logout}
